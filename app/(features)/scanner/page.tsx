...
export default function ScannerPage() {
  ...
  // Add a new state variable for API status after the other state variables
  ...
  // Simulate scanning process with stages
  const simulateScan = () => {
    setIsScanning(true)
    setScanProgress(0)
      ...
      setScanProgress((prev) => {
        ...
        if (newProgress === 20) {
          ...
          setScanResult({
            ...
            description:
              "Subspesies harimau yang hanya ditemukan di Pulau Sumatera, dengan ciri khas garis-garis hitam yang lebih rapat dan rambut yang lebih gelap dibandingkan subspesies harimau lainnya. Merupakan salah satu dari enam subspesies harimau yang masih bertahan hingga saat ini.",
            characteristics: {
              Berat: "100-140 kg",
              Panjang: "2.2-2.5 m",
              Tinggi: "75-90 cm",
  ...
  // Add a useEffect to simulate checking API status
  useEffect(() => {
    ...
    const checkApiStatus = async () => {
      ...
      // Simulate 90% chance of API being ready (for demo purposes)
            ...
            {Array.from({ length: 50 }).map((_, i) => (
              <motion.div
                ...
                className={`absolute w-3 h-3 rounded-full bg-${
                  ["emerald", "green", "teal", "yellow"][Math.floor(Math.random() * 4)]
